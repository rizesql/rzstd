...
impl<R: rzstd_io::Reader> Context<'_, R> {
    pub fn execute_sequences(&mut self, block_size: u32) -> Result<(), Error> {
        ...
        for seq in &self.sequences_buf {
            ...
            let next_lit_idx = lit_idx.checked_add(lit_len).ok_or(Error::Corruption)?;
...
fn update_offset_hist(
    ...
    let offset = match of_raw {
        ...
        3 => {
            let of = if lit_len == 0 {
                history[0].checked_sub(1).ok_or(Error::Corruption)?
            } else {
                history[2]
            };
        ...
        _ => {
            let of = (of_raw as usize).checked_sub(3).ok_or(Error::Corruption)?;
